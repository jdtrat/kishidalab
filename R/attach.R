...
pkgs <- c("pRpt", "sborgr", "iapsr")

kishidalab_attach <- function() {
  # Create `to_load` which is a character vector of all kishidalab
  ...
  # kishidalab with the package version on the right-hand side
  load_header <- cli::rule(
    left = crayon::bold("Attaching packages"),
    right = paste0("kishidalab ", package_version("kishidalab"))
  )

  # Return a character string containing the package version for each of kishidalab's constituents
...
kishidalab_detach <- function() {
  pak <- paste0("package:", c(pkgs, "kishidalab"))
  lapply(pak[pak %in% search()], detach, character.only = TRUE)
  invisible()
}

#' List all packages imported by kishidalab
#'
#' @export
#'
#' @examples
#' kishidalab_packages()
kishidalab_packages <- function() {
  # get all imports from kishidalab's package description file
  raw <- utils::packageDescription("kishidalab")$Imports
